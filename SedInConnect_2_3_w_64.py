...
class Connectivity_gui(QtGui.QMainWindow):
    ...
    def CavalliConnectivityout (self, dtm_f, c_s, w_f, out_ic, sink_flag = 0):#I will flag sink_are to 1 in case I need to retrieve the fdir_8 from the sink analysis    
        ...
        tif_ar = tif_ar.astype(float)
        band=tif.GetRasterBand(1)#bands retrieving
        geoinf = tif.GetGeoTransform()#tupla che contiene upper left coordinates, cellsize
        proj=tif.GetProjection() #sistema di riferimento
        tipo=tif_ar.dtype
        del(tif)
        #
        #
        ...
        #cell_size
        cell_s=numpy.float32(str(c_s)) #creo un numero pari alla risoluzione desiderata; importante che resti float32, per omogeneita'
        #
        ...
        if sink_flag == 0:
            ...
            # trasformo il raster 8 direzioni in Array(vettore 1D) e converto i nodata in NaN di numpy
            tif_fdir8_ar=tif_fdir8.ReadAsArray()
